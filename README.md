...
- The `public static void main` line that introduces the function.
- Parentheses around the argument for the `for` loop.
- Declaring variable `i = 1` within the argument for the `for` loop.
- Defining the range of `i` and the incrementation within the argument for the `for` loop.
- Defining specific Boolean variables before using them as part of `if` and `else` conditional statements. 
